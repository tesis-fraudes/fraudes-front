...
interface ModelHistoryProps {
  ...
  onDownloadModel: (model: ModelHistoryItem) => void;
  onDeleteModel: (model: ModelHistoryItem) => void;
...
export function ModelHistory({ 
  ...
  const getStatusBadge = (status: ModelHistoryItem['status']) => {
    switch (status) {
      ...
      case 'training':
        return <Badge variant="outline" className="border-blue-600 text-blue-600 bg-blue-50">Entrenando</Badge>;
      case 'error':
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Avatar className="h-6 w-6">
                            <AvatarImage src={`/avatars/${model.uploadedBy.toLowerCase()}.jpg`} />
                            <AvatarFallback className="text-xs">
                              {model.uploadedBy.split(' ').map(n => n[0]).join('')}
                            </AvatarFallback>
                          </Avatar>
