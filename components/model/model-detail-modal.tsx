      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <div><b>Versión:</b> {model.version}</div>
            <div><b>Precisión:</b> {model.accuracy !== undefined ? `${model.accuracy.toFixed(1)}%` : "N/A"}</div>
            <div><b>Subido por:</b> {model.uploadedBy}</div>
            <div><b>Tamaño:</b> {model.fileSize ? `${(model.fileSize / 1024).toFixed(2)} MB` : "N/A"}</div>
            <div><b>Estado:</b> {model.status}</div>
          </div>
